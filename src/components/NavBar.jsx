...
const NavBar = () => {
  ...
  return (
    <div className="z-10 flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed">
      <div>
        <h1 className="text-5xl ml-2 font-bold select-none">Diwas</h1>
      </div>
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          ...
          >
            <Link to={link} smooth duration={800} onClick={handleClick}>
              {link}
            </Link>
          </li>
        ))}
      ...
      <div
        onClick={handleClick}
        className="cursor-pointer pr-4 z-200 text-gray-500 md:hidden"
      >
